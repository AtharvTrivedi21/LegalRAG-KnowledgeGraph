"""
Phase 4 v2: Streamlit UI with structured answers and Cited references (Acts, Articles, Sections, Cases).
"""
...
def main() -> None:
    st.set_page_config(page_title="Phase 4 v2 – Graph-Constrained LegalRAG", layout="wide")

    st.title("Phase 4 v2 – Graph-Constrained LegalRAG")
    st.write(
        "Structured answers and cited references (Acts, Articles, Sections, Cases). "
    ...
    st.subheader("Query")
    # with st.expander("Example queries to try (click to copy)", expanded=False):
    #     st.markdown(
    #         "1. **Explicit provision:** `Explain Article 14 of the Constitution`  \n"
    #         "2. **Explicit section:** `What does Section 302 of BNS say?`  \n"
    #         "3. **Incident-style:** `Someone entered my home and stole my property`"
    #     )
    query = st.text_area(
        "Enter a legal question or describe an incident (e.g. 'Explain Article 14' or 'Someone entered my home and stole my property')",
        height=120,
    )
    ...
    if st.button("Run Graph-Constrained RAG", type="primary"):
        ...
        # Show formal legal query used for retrieval (from query rephrase step)
        ...
        # Cited references: show only when the answer indicates relevance
        act_ids = []
        ...
        if not no_applicable_laws:
            with st.expander("Cited references — Acts"):
                if not act_ids:
                    ...
                    st.caption("Act IDs from context could not be resolved in the graph (or Neo4j display unavailable).")
                    for aid in act_ids:
                        st.write(f"- {aid}")
                else:
                    for a in acts_data:
                        st.write(f"- **{a.get('act_id', '')}** — {a.get('act_name', '') or '(no name)'}")
        ...
        for art in graph_metadata.get("articles") or []:
            ...
            if aid and aid not in articles_seen:
                ...
                if len((art.get("full_text") or "")) > 200:
                    text += "..."
                articles_seen[aid] = {"article_number": art.get("article_number"), "act_id": art.get("act_id"), "snippet": text}
            ...
            with st.expander("Cited references — Articles"):
                if not articles_seen:
                    ...
                    for aid, info in articles_seen.items():
                        ...
                        st.write(f"- **{aid}** (Art. {num}, Act: {act_id})")
                        if info.get("snippet"):
        ...
        for sec in graph_metadata.get("sections") or []:
            ...
            if sid and sid not in sections_seen:
                ...
                if len((sec.get("full_text") or "")) > 200:
                    text += "..."
                sections_seen[sid] = {"section_number": sec.get("section_number"), "act_id": sec.get("act_id"), "snippet": text}
            ...
            with st.expander("Cited references — Sections"):
                if not sections_seen:
                    ...
                    for sid, info in sections_seen.items():
                        ...
                        act_id = info.get("act_id") or "-"
                        st.write(f"- **{sid}** (Sec. {num}, Act: {act_id})")
                        if info.get("snippet"):
                            st.caption(info["snippet"][:200] + ("..." if len(info.get("snippet", "")) > 200 else ""))
        ...
        with st.expander("Retrieved text snippets"):
            for stype, by_id in grouped_sources.items():
                ...
                for sid, info in by_id.items():
                    with st.expander(f"{sid} (score ~ {info.get('max_score', 0.0):.3f})"):
