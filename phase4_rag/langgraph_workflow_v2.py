"""
Phase 4 v2: LangGraph workflow with structured answers and internal-label stripping.

Same graph as phase4_rag.langgraph_workflow; only the answer_generator prompt
and post-processing differ (structured format + strip [ARTICLE/SECTION FROM KNOWLEDGE GRAPH]).
"""
...
class WorkflowState(TypedDict, total=False):
    user_query: str
    legal_query: Optional[str]  # LLM-rephrased formal legal query used for vector retrieval
...
def node_query_rephrase(state: WorkflowState) -> WorkflowState:
    """Rephrase user query into formal legal language for better vector retrieval."""
    ...
    try:
        messages = [
            ChatMessage(role="user", content=REPHRASE_PROMPT.format(user_query=user_query)),
        ]
        legal = chat_completion(messages).strip()
        state["legal_query"] = legal if legal else user_query
    except OllamaError:
        state["legal_query"] = user_query  # fallback to raw query
    return state


def node_vector_retriever(state: WorkflowState) -> WorkflowState:
    # Use rephrased legal query for retrieval when available; else raw user query
...
def _build_system_prompt_v2() -> str:
    return (
        ...
        "Under 'Applicable laws / provisions', you MUST list and cite every section_id and article_id from the context that is relevant to the question. "
        "Do not say 'none stated' or 'none explicitly stated' if the context contains any SECTION or ARTICLE text—identify and cite the applicable ones (e.g. BNS sections on theft, housebreaking, trespass; Constitution articles). "
        ...
        "Use brief bullets or short paragraphs under each. If a section does not apply (e.g. no case law), say so briefly. "
        ...
        "cite sources only by article_id, section_id, or case_id (e.g. Constitution_Art_14, BNS_Sec_41).\n"
    )
...
def _strip_internal_labels(text: str) -> str:
    """Remove [ARTICLE FROM KNOWLEDGE GRAPH] and [SECTION FROM KNOWLEDGE GRAPH] from model output."""
...
def _fix_markdown_layout(text: str) -> str:
    """Ensure ## headers are followed by newlines so content renders on separate lines."""
    # Ensure Summary has ## if model omitted it
    if re.match(r"^Summary\s+", text, re.I):
        text = "## " + text
    # Insert newline after each ## heading when content immediately follows (e.g. "## Summary The" -> "## Summary\n\nThe")
    text = re.sub(r"(##\s+[^\n#]+?)\s+(?=[A-Z])", r"\1\n\n", text)
...
def _answer_indicates_no_relevant(answer: str) -> tuple[bool, bool]:
    """Parse answer to detect if applicable laws and/or case law are reported as none. Returns (no_laws, no_cases)."""
    import re as _re
...
def _remove_empty_sections(text: str) -> str:
    """Remove ## X blocks that say 'None found' or similar."""
    import re as _re
...
def build_app():
    """Build and return the Phase 4 v2 LangGraph app."""
